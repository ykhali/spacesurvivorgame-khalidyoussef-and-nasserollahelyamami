...
function runGame() {
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (b.owner === "PLAYER") {
      for (let j = enemies.length - 1; j >= 0; j--) {
        if (b.hits(enemies[j])) {
          if (enemies[j].takeDamage(b.damage)) {
            // Use bullet damage
            // Explosion
  ...
  // Spawn random food
  if (frameCount % 600 === 0) {
    // Every 10 seconds (Energy)
    foods.push(
      new Food(random(50, width - 50), random(50, height - 50), "ENERGY"),
    );
  }
  if (frameCount % 300 === 0) {
    // Every 5 seconds (Snake Food)
    foods.push(
      new Food(random(50, width - 50), random(50, height - 50), "BIOMASS"),
    );
  }
  // Weapon Powerup (Rare, every 20 seconds)
  if (frameCount % 1200 === 0) {
    foods.push(
      new Food(random(50, width - 50), random(50, height - 50), "WEAPON"),
    );
  }
...
function keyPressed() {
  ...
  if (key === "p" || key === "P") {
    allies.push(
      new Ally(player.pos.x + random(-50, 50), player.pos.y + random(-50, 50)),
    );
  }
